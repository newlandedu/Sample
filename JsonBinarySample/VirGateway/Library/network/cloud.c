...
void* unpacket_cmd_req(cJSON* msg)
{
	...
	if(cmd_id && cmd_id->type != cJSON_String){
		NEWLAND_CLOUD_ERR("msg_id->type(%d) error\n", cmd_id->type);
	...
	}else if(data && data->type == cJSON_Array){
		array = cJSON_GetArrayItem(data,1);
		iten = cJSON_GetObjectItem(array,"Item2");
		if(iten == NULL)
			{
				NEWLAND_CLOUD_ERR("iten(Item2) is NULL\r\n");
				while(1);
			}
			else
			{
				cmd_req->data = malloc(sizeof(int));
				*((int*)cmd_req->data) = (*(iten->valuestring) - '0');
				cmd_req->data_type = CMD_DATA_TYPE_NUM;
			}	
	...
	cmd_req->cmd_id = atoi(cmd_id->string);
